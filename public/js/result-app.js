...
function render(){
 ...
 if(state.answer_issues?.length){root.innerHTML=`<section class="provisional-reading"><h1 class="result-headline">Review Your Answers for a Clearer Reading</h1><p>We could not prepare a reliable reading from these answers. Please review and edit your written answers with short, clear sentences. Optional writing can be left blank.</p></section><div class="restart-actions"><a class="btn-unlock" href="quiz.html">Review My Answers →</a></div>`;wireReadingActions();return;}
 ...
 ensureVoiceTriggered();
  if(state.free_testing&&['ai','guided','guided_fallback'].includes(state.source)&&!freePreviewRequested){freePreviewRequested=true;api('free-preview',{}).then(next=>{state=next;renderPreserving();}).catch(error=>say(error.message));}
  ...
  <nav class="reading-nav" aria-label="Your personal space"><a href="#readingChapters">Your reading</a>${p?'<a href="#personalAudio">Your audio</a><a href="#personalPlan">14-Day Plan</a>':individual&&!state.free_testing?'<a href="#continueReading">Continue your path</a>':''}<a href="#delivery">My account</a></nav>
...
function recommendation(){
 ...
 const diagHtml=rec?.leak_diagnosis?`<div class="vessel-diagnosis-card" style="background:rgba(218,203,166,0.08);border:1px solid rgba(218,203,166,0.35);border-radius:8px;padding:1.25rem 1.5rem;margin-bottom:1.5rem;text-align:left;"><span class="vessel-tag" style="color:var(--gold,#d4af37);font-weight:700;display:inline-block;margin-bottom:0.5rem;">✦ RABBI DAVID'S PASTORAL DIAGNOSIS</span><p style="font-size:1.05rem;line-height:1.6;color:var(--text,#f5f5f5);font-style:italic;margin:0 0 0.6rem 0;">"${esc(rec.leak_diagnosis)}"</p><p style="font-size:0.9rem;color:var(--text-muted,#b0b0b0);margin:0;">Recommended Vessel Companion: <strong style="color:var(--gold-light,#e2bf61);">${esc(rec.book?.title||'')}</strong></p></div>`:'';
 return `<section class="book-library" id="readingBookLibrary" aria-label="Book library" aria-roledescription="carousel" aria-live="off">${diagHtml}<div class="book-library-top"><span class="vessel-tag">${rec?'RECOMMENDED ANCESTRAL TEXT':'EXPLORE THE BOOK LIBRARY'}</span><div class="book-library-controls"><button type="button" data-library="previous" aria-label="Previous book">←</button><button type="button" data-library="next" aria-label="Next book">→</button></div></div><div class="book-library-viewport"><div class="book-library-slide"><p>Loading your recommended book…</p></div></div><div class="book-library-bottom"><span data-library-count></span><button type="button" data-library="pause" aria-label="Pause automatic book rotation">Pause</button></div><a class="funnel-secondary library-link" href="catalog.html">Explore the Book Library →</a><p class="library-note">Books are optional and sold separately from your personal reading.</p></section>`;
}

function wire(){wireReadingActions();globalThis.RabbiBookCarousel?.mount(document.getElementById('readingBookLibrary'),state.recommendation,bookId=>act(async()=>{state=await api('owned',{owned:[...state.owned,bookId]});render();}));document.getElementById('retryPlan')?.addEventListener('click',()=>act(async()=>{state=await api('plan-retry',{consent:true});render();}));document.querySelectorAll('[data-tier]').forEach(b=>b.addEventListener('click',()=>offer(b.dataset.tier)));document.getElementById('generateVoice')?.addEventListener('click',()=>act(async()=>{state=await api('voice',{});render();}));document.getElementById('alreadyOwn')?.addEventListener('click',e=>act(async()=>{state=await api('owned',{owned:[...state.owned,e.target.dataset.book]});render();}));document.getElementById('contactForm')?.addEventListener('submit',e=>{e.preventDefault();act(async()=>{state=await api('contact',{email:document.getElementById('deliveryEmail').value,marketing:document.getElementById('marketing').checked});say('Your preferences are saved.');});});}
function offer(tier){
 act(async()=>{
  state=await api('demo-tier',{tier:'personal'});
  render();
 });
}
