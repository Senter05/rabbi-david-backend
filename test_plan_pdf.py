...
class PlanPDFTests(unittest.TestCase):
    ...
    def test_exact_eighteen_pages_and_day_order(self):
        with fitz.open(stream=self.pdf, filetype='pdf') as pdf:
            ...
            self.assertIn('Sources & the Modern Exercises', pdf[17].get_text())
                ...
                for block in page.get_text('dict')['blocks']:
                    for line in block.get('lines', []):
                        for span in line['spans']:
                            ...
                            if 110 < y0 < 720 and len(span['text']) > 80 and page.number != 17:
    ...
    def test_sources_are_real_library_entries_not_model_urls(self):
        ...
        with fitz.open(stream=plan_pdf(data), filetype='pdf') as pdf:
            ...
            self.assertIn(SOURCES[1]['title'], pdf[17].get_text())
    ...
    def test_realistic_maximum_provider_word_counts_fit_eighteen_pages(self):
